...
public class VulnerableAppRestController {
  ...
  public VulnerableAppRestController(
      IEndPointsInformationProvider getAllSupportedEndPoints, @Value("${server.port}") int port) {
  ...
  /**
   * This Endpoint is exposed to help the scanners in finding the Vulnerable EndPoints. Here we are
   * not using any library as we need a very basic sitemap and we don't want to make VulnerableApp
   * heavy.
   *
  ...
  public String sitemapForPassiveScanners() throws JsonProcessingException, UnknownHostException {
    ...
    StringBuilder xmlBuilder =
        new StringBuilder(
            FrameworkConstants.GENERAL_XML_HEADER + FrameworkConstants.SITEMAP_URLSET_TAG_START);
