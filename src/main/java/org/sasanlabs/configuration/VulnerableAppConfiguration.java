...
public class VulnerableAppConfiguration {
  ...
  private static final List<String> MAX_FILE_UPLOAD_SIZE_OVERRIDE_PATHS =
      List.of("/" + UnrestrictedFileUpload.CONTROLLER_PATH + "/" + LevelConstants.LEVEL_9);
  ...
  /**
   * DB Configuration Below configuration is done to restrict the Application user rights and not to
   * give admin access rights to it. This is quite important because in case of any Vulnerability in
   * application it reduces the impact of the destruction because of the vulnerability.
   */
  @Bean
  @Primary
  ...
  /**
   * Customized MultipartFilter bean disables default max upload size for multipart files and their
   * overall requests, for select paths. See {@link
